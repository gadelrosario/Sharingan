...
class InjuryIntelligence423Tests(unittest.TestCase):
    def test_injury_decision_contracts(self):
        ...
        self.assertEqual((payload['passCount'],payload['failCount']),(8,0))

    def test_production_score_and_debug_use_injury_adjustment(self):
        app=(ROOT/'js/app.js').read_text(encoding='utf-8')
        final=app.split('function finalPickScore',1)[1].split('function sharinganIconMarkup',1)[0]
    ...
    def test_empty_snapshot_applies_unknown_not_healthy(self):
        snapshot=json.loads((ROOT/'data/injuries_2026.json').read_text(encoding='utf-8'))
        self.assertEqual(snapshot['records'],[])
        script="""const i=require('./js/injury-intelligence-v1'),p=[{id:1},{id:2}],s=require('./data/injuries_2026.json'),r=i.applySnapshot(p,s);if(r.unknown!==2||p.some(x=>x.injury.status!=='UNKNOWN'))process.exit(1);"""
