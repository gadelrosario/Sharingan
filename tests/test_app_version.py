...
"""
        ...
        self.assertEqual(json.loads(result.stdout), {"phase": "Jōnin", "milestone": "4.2", "label": "Jōnin 4.2"})
...
"""
    ...
    def test_one_browser_source_owns_visible_version_literal(self):
        ...
        self.assertEqual(version.count("label:'Jōnin 4.2'"), 1)
        for source in (html, app, manifest, worker):
            self.assertNotIn("Jōnin 4.2", source)
    ...
    def test_offline_cache_contains_canonical_metadata(self):
        worker = (ROOT / "service-worker.js").read_text(encoding="utf-8")
        app = (ROOT / "js" / "app.js").read_text(encoding="utf-8")
        self.assertIn('./js/app-version.js?v=1.0.2', worker)
