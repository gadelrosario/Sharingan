...
"""
        ...
        self.assertEqual(json.loads(result.stdout), {"phase": "Jōnin", "milestone": "4.4.7", "label": "Jōnin 4.4.7"})
...
"""
    ...
    def test_one_browser_source_owns_visible_version_literal(self):
        ...
        self.assertEqual(version.count("label:'Jōnin 4.4.7'"), 1)
        for source in (html, app, manifest, worker):
            self.assertNotIn("Jōnin 4.4.7", source)
    ...
    def test_offline_cache_contains_canonical_metadata(self):
        worker = (ROOT / "service-worker.js").read_text(encoding="utf-8")
        app = (ROOT / "js" / "app.js").read_text(encoding="utf-8")
        self.assertIn('./js/app-version.js?v=1.0.23', worker)
        self.assertIn('fantasy-hq-jonin-4-4-7-faab-intelligence', worker)
        self.assertIn('service-worker.js?v=jonin_4_4_7_faab_intelligence', app)
        self.assertIn('css/app.css?v=4.4.7.1', worker)
        ...
        self.assertIn('js/app.js?v=4.4.7.1', worker)
