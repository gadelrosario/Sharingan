...
"""
        ...
        self.assertEqual(json.loads(result.stdout), {"phase": "Jōnin", "milestone": "4.4.2", "label": "Jōnin 4.4.2"})
...
"""
    ...
    def test_one_browser_source_owns_visible_version_literal(self):
        ...
        self.assertEqual(version.count("label:'Jōnin 4.4.2'"), 1)
        for source in (html, app, manifest, worker):
            self.assertNotIn("Jōnin 4.4.2", source)
    ...
    def test_offline_cache_contains_canonical_metadata(self):
        ...
        self.assertIn('fantasy-hq-jonin-4-4-2', worker)
        self.assertIn('service-worker.js?v=jonin_4_4_2_waiver_intelligence', app)
        self.assertIn('css/app.css?v=4.4.2', worker)
        self.assertIn('js/app.js?v=4.4.2', worker)
