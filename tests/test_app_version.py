...
"""
        ...
        self.assertEqual(json.loads(result.stdout), {"phase": "Jōnin", "milestone": "4.4.15", "label": "Jōnin 4.4.15"})
...
"""
    ...
    def test_one_browser_source_owns_visible_version_literal(self):
        ...
        self.assertEqual(version.count("label:'Jōnin 4.4.15'"), 1)
        for source in (html, app, manifest, worker):
            self.assertNotIn("Jōnin 4.4.15", source)
    ...
    def test_offline_cache_contains_canonical_metadata(self):
        ...
        app = (ROOT / "js" / "app.js").read_text(encoding="utf-8")
        html = (ROOT / "index.html").read_text(encoding="utf-8")
        self.assertIn('js/app-version.js?v=1.0.35', html)
        self.assertIn('./js/app-version.js?v=1.0.35', worker)
        self.assertIn('fantasy-hq-jonin-4-4-15-lineup-workspace', worker)
        self.assertIn('service-worker.js?v=jonin_4_4_15_lineup_workspace_1', app)
        self.assertIn('css/app.css?v=4.4.15-lineup-workspace-1', worker)
        ...
        self.assertIn('js/season-current-week-evidence-v1.js?v=1.2.1', worker)
        self.assertIn('js/app.js?v=4.4.15-lineup-workspace-1', worker)
