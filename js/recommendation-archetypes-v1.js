...
(function(root){
  ...
  function assign(input=[]){
    ...
    const upside=by(rows.filter(qualifiesUpside),row=>(finite(row.upsideScore)??0)+(row.leagueBreaker===true?10:0)+(finite(row.upsideAdjustment)??0));claim(upside,'Highest Upside');
    ...
    return Object.freeze({labels,secondary,valueFound:Boolean(value),upsideFound:Boolean(upside)});
  }
