(function(root,factory){
  ...
  const weeklyNumbers=(records,path)=>{const distinct=new Map();records.filter(record=>record.week!==null).map(record=>({season:record.season,week:record.week,observedAt:record.observedAt,value:path.split('.').reduce((item,key)=>item?.[key],record),id:record.evidenceId})).filter(row=>row.value!==null&&row.value!==undefined&&Number.isFinite(Number(row.value))).forEach(row=>distinct.set(`${row.season}|${row.week}`,{...row,value:Number(row.value)}));return[...distinct.values()].sort((a,b)=>a.season-b.season||a.week-b.week||String(a.observedAt).localeCompare(String(b.observedAt)))};
  ...
  class DiscoveryBreakoutRadarV1{
    ...
    _core(player){
      ...
      else if(identity.identityType==='SEASON_ONLY'&&distinctWeeks){classification='STASH_WATCH';strength='WEAK';timing='WAIT'}
      else if(distinctWeeks>=3){classification='STABLE';strength='WEAK';timing='IGNORE'}
    ...
    buildView(results=[]){const available=results.filter(result=>result.classification!=='INSUFFICIENT_EVIDENCE'),topSignals=available.filter(result=>['BREAKOUT','ROLE_GROWTH'].includes(result.classification)).slice(0,3),emerging=available.filter(result=>['EMERGING','OPPORTUNITY_WATCH'].includes(result.classification)).slice(0,5),watchlist=[],fadingNoise=available.filter(result=>['FADING','NOISE'].includes(result.classification));return freeze({schemaVersion:SCHEMA_VERSION,topSignals:freeze(topSignals),emerging:freeze(emerging),watchlist:freeze(watchlist),fadingNoise:freeze(fadingNoise),suppressedNoSignal:results.filter(result=>['STABLE','STASH_WATCH','INSUFFICIENT_EVIDENCE'].includes(result.classification)).length,insufficientEvidence:results.filter(result=>result.classification==='INSUFFICIENT_EVIDENCE').length,total:results.length,recommendationAuthority:false,transactionAuthority:false})}
  }
