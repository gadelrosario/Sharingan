(function(root,factory){
  ...
  function outlook(lineup){
    const starters=(lineup?.starters||[]).filter(row=>row?.player),components=[],unavailable=(lineup?.starters||[]).some(row=>!row?.player)?['EMPTY_STARTER_SLOT']:[];
    ...
    starters.forEach(row=>{const evidence=row.player.currentWeek,gameState=evidence?.game?.state,actual=evidence?.actual?.value,projectionValue=evidence?.projection?.value;if(gameState===GAME_STATE.FINAL&&evidence?.actual?.supported===true&&finite(actual)!==null)components.push(freeze({playerId:playerId(row.player),slot:row.slot,value:actual,kind:'ACTUAL',source:'YAHOO_PLAYER_POINTS'}));else if(gameState===GAME_STATE.PRE_GAME&&evidence?.projection?.supported===true&&finite(projectionValue)!==null)components.push(freeze({playerId:playerId(row.player),slot:row.slot,value:projectionValue,kind:'PROJECTION',source:evidence.projection.source}));else unavailable.push(`${playerId(row.player)||row.player.name}:${gameState||'UNKNOWN'}`);});
