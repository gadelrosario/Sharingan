(function(root,factory){
  ...
  const GAME_STATE=Object.freeze({PRE_GAME:'PRE_GAME',LIVE:'LIVE',FINAL:'FINAL',LOCKED_UNKNOWN:'LOCKED_UNKNOWN',UNKNOWN:'UNKNOWN'});
  ...
  function projection(player,{week,scoringFormat,fetchedAt,now=Date.now(),provider='Yahoo',demo=false}={}){
    const value=finite(player?.projection??player?.projectedPoints??player?.yahooProjection),source=clean(player?.projectionSource)||(value!==null&&provider==='Yahoo'?'YAHOO_PLAYER_PROJECTED_POINTS':demo&&value!==null?'SANITIZED_FIXTURE':null),sourceWeek=finite(player?.projectionWeek??player?.week??week),sourceFormat=normalizeFormat(player?.projectionScoringFormat??player?.scoringFormat??scoringFormat),expectedFormat=normalizeFormat(scoringFormat),timestamp=iso(player?.projectionFetchedAt||player?.evidenceFetchedAt||fetchedAt),state=freshness('projection',timestamp,now),weekMatch=value!==null&&sourceWeek===Number(week),formatMatch=value!==null&&Boolean(sourceFormat&&expectedFormat&&sourceFormat===expectedFormat),supported=value!==null&&Boolean(source)&&weekMatch&&formatMatch&&[FRESHNESS.CURRENT,FRESHNESS.AGING].includes(state);
    return freeze({value:supported?value:null,rawValue:value,source:source||null,week:sourceWeek,scoringFormat:sourceFormat,fetchedAt:timestamp,freshness:state,confidence:supported?(state===FRESHNESS.CURRENT?'HIGH':'MEDIUM'):'UNAVAILABLE',supported,reason:supported?null:value===null?'MISSING_VALUE':!source?'MISSING_SOURCE':!weekMatch?'WEEK_MISMATCH':!formatMatch?'SCORING_FORMAT_MISMATCH':state==='STALE'?'STALE':'UNKNOWN_FRESHNESS'});
  }
  function game(player,{now=Date.now(),fetchedAt}={}){
  ...
  function enrichPlayer(player,context={}){
    const resolved=identity(player),id=resolved.canonicalPlayerId,gameEvidence=game(player,context),projectionEvidence=projection(player,context),availabilityEvidence=availability(player,resolved,context),injuryRecord=context.injuries?.get(id)||null,injuryEvidence=injury(player,injuryRecord,context),usageEvidence=usage(context.evidenceStore,id,{season:context.season,week:context.week,now:context.now}),actual=finite(player?.actualPoints),completed=gameEvidence.state===GAME_STATE.FINAL,live=gameEvidence.state===GAME_STATE.LIVE,contribution=completed&&actual!==null?freeze({value:actual,source:'YAHOO_ACTUAL_POINTS',kind:'ACTUAL'}):gameEvidence.state===GAME_STATE.PRE_GAME&&projectionEvidence.supported?freeze({value:projectionEvidence.value,source:projectionEvidence.source,kind:'PROJECTION'}):null;
    const currentWeek=freeze({schema:SCHEMA,identity:resolved,season:Number(context.season)||null,week:Number(context.week)||null,lineupSlot:clean(player?.rosterSlot)||null,eligiblePositions:freeze([...(player?.eligiblePositions||[])].map(position).filter(Boolean)),availability:availabilityEvidence,opponent:clean(player?.opponent)||null,game:gameEvidence,injury:injuryEvidence,projection:projectionEvidence,actual:freeze({value:actual,source:actual!==null?'YAHOO_PLAYER_POINTS':null,supported:actual!==null}),usage:usageEvidence,outlookContribution:contribution,liveExpectationSupported:live&&finite(player?.liveExpectedPoints)!==null,recommendationAuthority:false,provenance:freeze({roster:context.provider==='Yahoo'?'Yahoo':context.demo?'Sanitized fixture':null,rosterFetchedAt:iso(context.fetchedAt),game:gameEvidence.source,projection:projectionEvidence.source,injury:injuryEvidence.source,usage:usageEvidence.available?'FantasyHQSeasonEvidenceV1':null})});
    return freeze({...player,canonicalPlayerId:id||player?.canonicalPlayerId||null,locked:gameEvidence.locked,injuryStatus:injuryEvidence.current?injuryEvidence.status:clean(player?.injuryStatus)||null,projection:projectionEvidence.supported?projectionEvidence.value:null,currentWeek});
  }
  function remapLineup(lineup,index){
    if(!lineup)return null;
    const rows=name=>(lineup[name]||[]).map(row=>freeze({...row,player:row?.player?index.get(lineupKey(row.player))||row.player:null}));
