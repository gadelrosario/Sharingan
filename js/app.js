...
async function init() {
  ...
  if (DOM.draftSlot) {
    for (let i = 1; i <= 10; i++) {
      ...
      DOM.draftSlot.appendChild(o);
    }
    DOM.draftSlot.value = 10;
  }
  renderManagerSetup();
...
function teamForPick(p) {
  let r = Math.ceil(p / 10),
    x = ((p - 1) % 10) + 1;
  return r % 2 ? x : 11 - x;
}
function info() {
  let r = Math.ceil(pick / 10),
    ip = ((pick - 1) % 10) + 1,
    next = pick;
  while (next <= TOTAL_PICKS && teamForPick(next) !== slot) next++;
  return { r, ip, until: Math.max(0, Math.min(TOTAL_PICKS, next) - pick) };
}
function available() {
...
function finalPickScore(p) {
  let key = `f:${intelligenceEpoch}:${p.id}`;
...
function aiScore(p, team) {
  ...
  if (profile === 'Early QB' && p.pos === 'QB' && c.QB < 1 && Math.ceil(pick / 10) <= 5) s += 28;
...
function randomizeManagerOrder() {
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    if (!node) continue;
    node.value = i === +(DOM.draftSlot?.value || 10) ? 'Gerard' : names[idx++];
  }
}
...
function buildYahooRecord() {
  ...
  return {
    ...
    picks: history.map(h => {
      ...
      return {
        overallPick: h.pick,
        round: Math.ceil(h.pick / 10),
        pickInRound: ((h.pick - 1) % 10) + 1,
        teamSlot: h.team,
...
function recentPicksMarkup() {
  ...
  return rows
    .map(h => {
      ...
      return `<div class="recentPick"><span class="meta">${h.pick}</span><div><b>${p?.name || 'Unknown'}</b><div class="meta">${positionKey(p || { pos: '' })} • ${nm}</div></div><span class="meta">${Math.ceil(h.pick / 10)}.${String(((h.pick - 1) % 10) + 1).padStart(2, '0')}</span></div>`;
    })
    .join('');
}
...
function renderPlayers() {
  ...
  let pool = available()
    ...
    .sort((a, b) => {
      const rankDifference = bestAvailableRank(a) - bestAvailableRank(b);
  ...
  const html =
    pool
      .map(player => {
        ...
        const displayedRank = player.overall || bestAvailableRank(player);
