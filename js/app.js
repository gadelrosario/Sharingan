...
let yahooSyncController=null,yahooDiscoveredLeagues=[],seasonDemoSnapshot=null,seasonEvidenceFixture=null,seasonInjuryOpportunityFixture=null,seasonEvidenceStore=null,seasonPlayerRegistry=null,seasonInjuryOpportunityEngine=null,seasonNflEvidenceImportStatus=null,seasonPage='home',seasonWeekOverride=null,seasonSelectedPlayer=null;
...
async function ensureSeasonEvidenceStore(){
  ...
  if(!seasonDemoEnabled()){
    ...
    try{
      const [evidenceResponse,registryResponse]=await Promise.all([fetch('data/season_evidence/nflverse_latest.json',{cache:'no-store'}),fetch('data/season_evidence/season_player_registry.json',{cache:'no-store'})]);
      ...
      const registryPlayers=seasonPlayerRegistry.evidencePlayers(),result=seasonEvidenceStore.importPayload(artifact,{players:[...players,...registryPlayers]}),registryStatus=seasonPlayerRegistry.status();
      seasonNflEvidenceImportStatus={state:result.accepted||result.idempotent?'IMPORTED':'EMPTY',provider:artifact.provider,season:artifact.season,weeks:artifact.weeks||[],generatedAt:artifact.generatedAt,providerUpdatedAt:artifact.providerUpdatedAt,records:result.accepted+result.idempotent,players:seasonEvidenceStore.status().players,rejected:result.rejected,sourceUrl:artifact.sourceUrl||null,seasonIdentities:registryStatus.seasonIdentities,newProviderDiscovered:registryStatus.autoVerified,registryQuarantined:registryStatus.quarantined,message:result.accepted||result.idempotent?'Validated local NFL evidence and Season identities imported in shadow mode.':'The local artifact contains no usable evidence.',recommendationAuthority:false};
    ...
    seasonInjuryOpportunityEngine=window.FantasyHQInjuryOpportunityIntelligenceV1?new FantasyHQInjuryOpportunityIntelligenceV1.InjuryOpportunityIntelligence(seasonEvidenceStore):null;return seasonEvidenceStore;
  }
  const [evidenceResponse,intelligenceResponse]=await Promise.all([fetch('tests/fixtures/season_evidence_4_4_5.json',{cache:'no-store'}),fetch('tests/fixtures/injury_opportunity_4_4_6.json',{cache:'no-store'})]);if(!evidenceResponse.ok||!intelligenceResponse.ok)throw new Error('Season intelligence fixtures are unavailable.');seasonEvidenceFixture=await evidenceResponse.json();seasonInjuryOpportunityFixture=await intelligenceResponse.json();seasonEvidenceStore=new FantasyHQSeasonEvidenceV1.SeasonEvidenceStore({asOf:seasonEvidenceFixture.asOf});seasonEvidenceStore.importPayload(seasonEvidenceFixture,{players:[...seasonEvidenceFixture.canonicalPlayers,...seasonInjuryOpportunityFixture.canonicalPlayers],aliases:seasonEvidenceFixture.aliases});seasonEvidenceStore.importPayload(seasonInjuryOpportunityFixture,{players:[...seasonEvidenceFixture.canonicalPlayers,...seasonInjuryOpportunityFixture.canonicalPlayers]});seasonPlayerRegistry=null;seasonNflEvidenceImportStatus={state:'DEMO',provider:'Sanitized fixture',season:2026,weeks:[1,2,3],records:seasonEvidenceStore.status().records,players:seasonEvidenceStore.status().players,seasonIdentities:0,newProviderDiscovered:0,registryQuarantined:0,message:'Sanitized demo evidence only.',recommendationAuthority:false};seasonInjuryOpportunityEngine=window.FantasyHQInjuryOpportunityIntelligenceV1?new FantasyHQInjuryOpportunityIntelligenceV1.InjuryOpportunityIntelligence(seasonEvidenceStore):null;return seasonEvidenceStore;
}
...
function seasonWaiverDecisionCard(pair,model,{identity='BEST CURRENT DECISION',compact=false}={}){
  ...
  const faab=seasonFaabEvaluation(model,pair);body.append(head,seasonEl('p','seasonDecisionReason',seasonWaiverExplanation(pair)),seasonEl('p','seasonFaabSummary',seasonFaabLabel(faab)));
...
function seasonWeeklyCommandInput(model){
  ...
  return{profileId:activeLeagueProfile?.id||model?.profileId,weeklyPhase:model?.snapshot?.weeklyPhase||(model?.demo?'WAIVER_WINDOW':'UNKNOWN'),yahooAuthority,waiver,faabByPlayer,startSit,teamFit,teamFitByPlayer,injuryOpportunity,waiverWindow:model?.snapshot?.waiverWindow||'BEFORE_WAIVERS',waiverInformationEvent:model?.snapshot?.waiverInformationEvent||null,startSitWindow:model?.snapshot?.startSitWindow||null,actionLimit:3,watchLimit:3,holdLimit:3};
}
function seasonWeeklyPlan(model){return window.FantasyHQWeeklyCommandCenterV1?FantasyHQWeeklyCommandCenterV1.orchestrate(seasonWeeklyCommandInput(model)):null}
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () =>
    navigator.serviceWorker
      .register('./service-worker.js?v=jonin_4_4_9_season_player_registry')
      .then(reg => reg.update())
