...
let yahooSyncController=null,yahooDiscoveredLeagues=[],seasonDemoSnapshot=null,seasonPage='home',seasonWeekOverride=null,seasonSelectedPlayer=null;
...
async function seasonStateForActiveProfile(){const live=currentYahooSeasonState(),demo=seasonDemoEnabled()?await loadSeasonDemo():null;return SeasonCommandCenterV1.resolveSeasonState({live,demo,entries:discoverRecoverableCompletedDrafts(),profile:activeLeagueProfile,canonicalPlayers:players})}
...
function openSeasonStartSitAnalysis(decision,model){const player=decision?.preferred||decision?.starter;if(!player)return;openSeasonPlayerDetails(player,model);const content=el('scanContent');if(!content)return;const detail=seasonEl('section','seasonWaiverDetail seasonStartSitAnalysis');detail.appendChild(seasonSectionHeader('START/SIT ANALYSIS'));const preferredScore=decision.preferred===decision.starter?decision.scores.starter:decision.scores.alternative,otherScore=decision.preferred===decision.starter?decision.scores.alternative:decision.scores.starter,evidence=preferredScore?.dimensions||{},facts=[['Decision',decision.state],['Lineup slot',decision.lineupSlot],['Preferred player',decision.preferred?.name||'Unresolved'],['Alternative',decision.other?.name||'Unresolved'],['Timing',decision.timingState],['Timing rationale',decision.timingReason],['Comparison rationale',decision.reason],['Confidence',`${Math.round(decision.confidence||0)} / 100`],['Meaningful edge',decision.edge===null?'Unknown':decision.edge.toFixed(1)],['Evidence coverage',`${Math.round((decision.evidenceCoverage||0)*100)}%`],['Current role',evidence.role??'Unscored'],['Opportunity',evidence.opportunity??'Unscored'],['Recent usage',evidence.recentUsage??'Unscored'],['Role stability',evidence.roleStability??'Unscored'],['Matchup',evidence.matchup??'Unscored'],['Floor',evidence.floor??'Unscored'],['Ceiling / upside',evidence.upside??'Unscored'],['Volatility',evidence.volatility??'Unscored'],['Injury uncertainty',evidence.injuryUncertainty??'Unscored'],['Workload uncertainty',evidence.workloadUncertainty??'Unscored'],['Scoring fit',evidence.scoringFit??'Unscored'],['Projection evidence',evidence.projection??'Unscored'],['Evidence freshness',preferredScore?.freshness||'Unknown'],['Role source',preferredScore?.provenance?.roleSource||'Unknown'],['Opportunity source',preferredScore?.provenance?.opportunitySource||'Unknown'],['Usage source',preferredScore?.provenance?.usageSource||'Unknown'],['Matchup source',preferredScore?.provenance?.matchupSource||'Unknown'],['Projection source',preferredScore?.provenance?.projectionSource||'Unknown'],['Alternative normalized score',otherScore?.score===null?'Unscored':otherScore.score.toFixed(1)]];const grid=seasonEl('div','seasonDetailGrid');facts.forEach(([label,value])=>{const item=seasonEl('div');item.append(seasonEl('small','',label),seasonEl('strong','',value));grid.appendChild(item)});detail.appendChild(grid);content.appendChild(detail)}
...
function openSeasonPlayerDetails(player,model){seasonSelectedPlayer=player;const content=el('scanContent');if(!content)return;content.innerHTML='';content.append(seasonSectionHeader(player.name||'Unknown player',seasonButton('Close',()=>closeScan(),'seasonTextButton')));const facts=[['Position',player.position||'Not provided'],['NFL team',player.sourceTeam||'Not provided'],['Yahoo roster slot',player.rosterSlot||'Available player'],['Yahoo status',player.availability||player.ownership||'Rostered'],['Injury status',player.injuryStatus||'No injury flag supplied'],['Draft origin',player.draftOrigin?`Drafted ${player.draftOrigin}`:model.linkedDraft?'No linked pick found':'No linked draft archive'],['TeamFit','Not yet scored'],['Rest-of-season value','Not yet scored'],['Championship Equity','Not yet scored'],['Start/Sit reasoning','Coming in 4.4.3']];const grid=seasonEl('div','seasonDetailGrid');facts.forEach(([label,value])=>{const item=seasonEl('div');item.append(seasonEl('small','',label),seasonEl('strong','',value));grid.appendChild(item)});content.appendChild(grid);el('scanModal')?.classList.remove('hidden')}
...
openSeasonPlayerDetails = function (player, model) {
  ...
  const evidence = pair.dimensions || {}, competition = pair.competition || {}, impact = seasonWaiverRosterImpact(pair), detail = seasonEl('section','seasonWaiverDetail');
  detail.appendChild(seasonSectionHeader('SHARINGAN VISION — WAIVER ANALYSIS'));
  const facts = [
    ['Availability', player.availability || player.ownership || 'Unknown'],
    ['Roster need fit', pair.rosterNeed || 'UNKNOWN'],
    ['Role signal', evidence.role ?? 'Unknown'],
    ['Opportunity evidence', evidence.opportunity ?? 'Unknown'],
    ['Upside', evidence.upside ?? 'Unknown'],
    ['Risk', evidence.risk ?? 'Unknown'],
    ['Suggested action', pair.action],
    ['Timing rationale', seasonWaiverTimingCopy(pair)],
    ['Suggested drop', pair.drop?.player?.name || 'No acceptable drop'],
    ['Drop protection', pair.drop?.state || 'UNRESOLVED'],
    ['Acquisition cost signal', evidence.acquisitionCost ?? 'Unknown'],
    ['Drop cost signal', pair.drop?.dropCost ?? 'Unknown'],
    ['Evidence coverage', `${Math.round((pair.confidence||0)*100)}%`],
    ['Starter demand', competition.starterNeedCount ?? 'Unknown'],
    ['Depth demand', competition.depthDemandCount ?? 'Unknown'],
    ['Comparable alternatives', competition.comparableAlternativeCount ?? 'Unknown'],
    ['Competition context', competition.reason || 'Unknown'],
    ['Championship Equity evidence', evidence.championshipEquity ?? 'Unknown'],
    ['Role source', pair.provenance?.roleSource || 'Unknown'],
    ['Opportunity source', pair.provenance?.opportunitySource || 'Unknown'],
    ['Ranking source', pair.provenance?.rankingSource || 'Unknown'],
    ['Availability source', pair.provenance?.availabilitySource || 'Unknown'],
    ['FAAB', intelligence?.faab?.label || 'FAAB: Not yet scored'],
    ['Before', impact.before],
    ['Move', impact.move],
    ['After', impact.after],
  ];
  const grid = seasonEl('div','seasonDetailGrid');
  facts.forEach(([label,value])=>{const item=seasonEl('div');item.append(seasonEl('small','',label),seasonEl('strong','',value));grid.appendChild(item)});
  detail.appendChild(grid);content.appendChild(detail);
};

if ('serviceWorker' in navigator) {
  window.addEventListener('load', () =>
    navigator.serviceWorker
      .register('./service-worker.js?v=jonin_4_4_3_start_sit_intelligence')
      .then(reg => reg.update())
