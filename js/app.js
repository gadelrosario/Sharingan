...
function seasonWaiverCard(model,{full=false}={}){const card=seasonEl('section','seasonPanel seasonWaiverPanel');card.append(seasonSectionHeader(full?'AVAILABLE PLAYERS':'BEST WAIVER TARGETS',full?null:seasonButton('View all ›',()=>showSeasonPage('waivers'),'seasonTextButton')));const info=seasonEl('p','seasonTrustNote',model.available.length?'Yahoo availability only • TeamFit, opportunity score, and FAAB are not yet evaluated.':'Yahoo supplied no available players for this snapshot.');card.appendChild(info);model.available.slice(0,full?model.available.length:4).forEach(player=>card.appendChild(seasonAvailableRow(player,model)));if(!model.available.length)card.appendChild(seasonEmpty('No available players','Sync Yahoo again or verify the players subsystem.'));return card}
...
function seasonFlightHero(model){const control=SeasonCommandCenterV1.flightControl(model),hero=seasonEl('section',`seasonHero ${model.phase.key}`);const left=seasonEl('div','seasonHeroSignal');left.append(seasonEl('span','seasonRadarGlyph','◉'));const copy=seasonEl('div','seasonHeroCopy');const eyebrow=seasonEl('div','seasonHeroEyebrow');eyebrow.append(seasonEl('b','',`FLIGHT CONTROL`),seasonStatusBadge(model.phase.label,'blue'));copy.append(eyebrow,seasonEl('h1','',`${control.action} — ${control.headline}`),seasonEl('p','',control.reason));const watch=seasonEl('aside','seasonWatch');watch.append(seasonEl('small','','NEXT WINDOWS TO WATCH'),seasonEl('p','','Waivers • Practice reports • Lineup lock'),seasonButton('View all season surfaces',()=>showSeasonPage('radar'),'seasonPrimaryButton'));hero.append(left,copy,watch);const metrics=seasonEl('div','seasonHeroMetrics');[['Team strength','Not yet scored'],['Start/Sit confidence','Not yet scored'],['Waiver opportunity','Not yet scored'],['Injury impact',model.objective.irCount?`${model.objective.irCount} on IR`:'No IR slot flags']].forEach(([label,value])=>{const item=seasonEl('div');item.append(seasonEl('small','',label),seasonEl('strong','',value));metrics.appendChild(item)});hero.appendChild(metrics);return hero}
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () =>
    navigator.serviceWorker
      .register('./service-worker.js?v=jonin_4_4_1d1_season_home_roster')
      .then(reg => reg.update())
