...
function decisionCardMarkup(model,{recommended=false}={}){
 ...
 return `<div class="decisionCard ${recommended?'recommendedDecision':'comparisonDecision'}">
  <div class="decisionHeader"><div><div class="heroEyebrow">WHO SHOULD I DRAFT?</div><h2>${safeInsightText(hero.name)}</h2><div class="heroIdentity">${safeInsightText(hero.identity)}</div></div><div class="confidenceAnswer"><span>HOW CONFIDENT ARE YOU?</span><strong>${safeInsightText(hero.confidence)}%</strong><small>${safeInsightText(hero.confidenceLabel)}</small></div></div>
  <div class="decisionReasons"><b>WHY?</b><ul>${reasons}</ul><div class="compactComparison">${safeInsightText(summary.comparison)}</div></div>
  <div class="canWaitAnswer"><span>CAN I WAIT?</span><strong>${safeInsightText(summary.wait.action)}</strong><b>Availability: ${safeInsightText(summary.wait.availability)}</b><small>${safeInsightText(summary.wait.conclusion)}</small></div>
