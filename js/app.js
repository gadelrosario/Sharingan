...
function concisePlayerComparison(player,primary,breakdown,primaryBreakdown,vision){
 ...
 return `Compared with ${alternative.name}: ${winner.name} ${tone}wins on ${reason}.`;
}
...
function playerDecisionModel(player,recs){
 ...
 const summary=window.FlightControlV1?FlightControlV1.decisionSummary({hero,vision,insight,comparison}):{action:'LEAN DRAFT',primary:hero.primary,reasons:[hero.primary.reason],comparison};
...
function decisionCardMarkup(model,{recommended=false}={}){
 ...
 return `<div class="decisionCard ${recommended?'recommendedDecision':'comparisonDecision'}">
  <div class="decisionHeader"><div><div class="heroEyebrow">${recommended?'RECOMMENDED PICK':'COMPARISON PICK'}</div><h2>${safeInsightText(hero.name)}</h2><div class="heroIdentity">${safeInsightText(hero.identity)}</div></div></div>
  <div class="compactAction"><strong>${safeInsightText(summary.action)}</strong><span>— ${safeInsightText(hero.confidence)}% confidence</span></div>
  <div class="decisionReasons"><b>Why:</b><ul>${reasons}</ul></div>
