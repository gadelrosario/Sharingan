...
function playerDecisionModel(player,recs){
 ...
 const summary=window.FlightControlV1?FlightControlV1.decisionSummary({hero,vision,insight}):{opportunity:{label:'Decision developing',reason:'More draft context is needed.'},availability:{label:'Uncertain',reason:'Availability guidance is unavailable.'},primary:hero.primary,reasons:[hero.primary.reason],whyNot:'Comparison guidance is unavailable.'};
...
function decisionCardMarkup(model,{recommended=false}={}){
 ...
 const reasons=summary.reasons.map(reason=>`<li>${safeInsightText(reason)}</li>`).join('');
 return `<div class="decisionCard ${recommended?'recommendedDecision':'comparisonDecision'}">
  <div class="decisionHeader"><div><div class="heroEyebrow">${recommended?'JŌNIN RECOMMENDATION':'COMPARISON VIEW'}</div><h2 class="scanLink" onclick="openScan(${p.id})">${safeInsightText(hero.name)}</h2><div class="heroIdentity">${safeInsightText(hero.identity)}</div></div><div class="heroConfidence"><strong>${safeInsightText(hero.confidence)}%</strong><span>Heuristic confidence</span><small>${safeInsightText(hero.confidenceLabel)}</small></div></div>
  <div class="decisionVerdict"><div><span>Decision</span><b>${safeInsightText(summary.opportunity.label)}</b></div><div><span>Primary driver</span><b>${safeInsightText(summary.primary.label)}</b></div><div><span>If you wait</span><b>${safeInsightText(summary.availability.label)}</b></div></div>
  <div class="decisionReasons"><b>Why this player?</b><ul>${reasons}</ul></div>
  <div class="decisionWait"><div><b>What happens if I wait?</b><span>${safeInsightText(summary.opportunity.reason)} ${safeInsightText(summary.availability.reason)}</span></div><div><b>Why not this player?</b><span>${safeInsightText(summary.whyNot)}</span></div></div>
  <div class="decisionActions"><button class="primary decisionDraftBtn" onclick="selectPlayer(${p.id},${slot})">Draft ${safeInsightText(p.name)}</button>${recommended?'':`<button class="ghost" onclick="selectCandidate(${p.id})">Back to Jōnin pick</button>`}<button type="button" class="scanBtn" onclick="openScan(${p.id})">Full player scan</button></div>
  <details class="advancedAnalysis"><summary>Advanced Analysis <span>Engine metrics and full explanations</span></summary><div class="advancedMetricStrip"><span>Mamba <b>${score}</b></span><span>Final Pick <b>${finalPickScore(p)}</b></span><span>Room Boost <b>${signedScore(roomBoost(p))}</b></span><span>Roster Fit <b>${signedScore(rosterFitModifier(p))}</b></span><span>Steal Risk <b>${risk}%</b></span><span>Stack <b>${safeInsightText(bp.stack.label)}</b></span><span>Handcuff <b>${safeInsightText(bp.hand.label)}</b></span><span>Exposure <b>${safeInsightText(bp.exp?bp.exp.text:'No concern')}</b></span></div>${joninInsightMarkup(insight,ccScored)}${sharinganVisionMarkup(vision)}<div class="tagrow">${tierBadge(p)}<span class="tag">${safeInsightText(p.pos)}${safeInsightText(p.posRank||'')}</span><span class="tag">${safeInsightText(p.team||'Team unavailable')}</span><span class="tag">Bye ${safeInsightText(p.bye??'—')}</span></div><div class="heroState">${safeInsightText(state.label)}</div></details>
 </div>`;
}
function alternativeDecisionMarkup(model,rank){
 const summary=window.FlightControlV1?FlightControlV1.comparisonSummary(model):{name:model.player.name,identity:model.player.pos,confidence:model.hero.confidence,opportunity:model.summary.opportunity.label,availability:model.summary.availability.label,reason:model.summary.primary.reason};
 return `<article class="alternativeDecision" onclick="selectCandidate(${model.player.id})" tabindex="0" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();selectCandidate(${model.player.id})}"><span class="rank">${rank}</span><div class="alternativeIdentity"><b>${safeInsightText(summary.name)}</b><small>${safeInsightText(summary.identity)}</small><p>${safeInsightText(summary.reason)}</p></div><div class="alternativeSignals"><span>${safeInsightText(summary.opportunity)}</span><small>${safeInsightText(summary.availability)}</small></div><button type="button" class="autoPickBtn" onclick="selectPlayer(${model.player.id},${slot});event.stopPropagation()">Draft</button></article>`;
}
