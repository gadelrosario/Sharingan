...
const QUALITY_REASONS=Object.freeze({INVALID_NUMERIC:'INVALID_NUMERIC',IMPOSSIBLE_NEGATIVE_COUNT:'IMPOSSIBLE_NEGATIVE_COUNT',IDENTITY_FAILURE:'IDENTITY_FAILURE',MALFORMED_ROW:'MALFORMED_ROW',DUPLICATE_OR_CONFLICT:'DUPLICATE_OR_CONFLICT'});
...
function errorReason(error){const detail=clean(error?.message||error);if(/must be numeric/.test(detail))return QUALITY_REASONS.INVALID_NUMERIC;if(/must be non-negative/.test(detail))return QUALITY_REASONS.IMPOSSIBLE_NEGATIVE_COUNT;return QUALITY_REASONS.MALFORMED_ROW}
function errorField(error){const match=clean(error?.message||error).match(/^([A-Za-z][A-Za-z0-9.]*) must be (?:numeric|non-negative)$/);return match?.[1]||null}
function quarantineReason(item={}){if(item.reasonCategory)return item.reasonCategory;if(['DUPLICATE_SOURCE_ID','DUPLICATE_CANONICAL_ATTACHMENT'].includes(item.reason))return QUALITY_REASONS.DUPLICATE_OR_CONFLICT;return QUALITY_REASONS.MALFORMED_ROW}
function countReasons(items=[],classifier=item=>item.reason){return Object.freeze(items.reduce((counts,item)=>{const reason=classifier(item)||QUALITY_REASONS.MALFORMED_ROW;counts[reason]=(counts[reason]||0)+1;return counts},{}))}
function countFieldReasons(items=[]){const counts={};items.forEach(item=>{const reason=quarantineReason(item),field=clean(item.field);if(!field)return;(counts[reason]??={})[field]=((counts[reason]||{})[field]||0)+1});return Object.freeze(Object.fromEntries(Object.keys(counts).sort().map(reason=>[reason,Object.freeze(Object.fromEntries(Object.entries(counts[reason]).sort(([left],[right])=>left.localeCompare(right))))]))) }
function normalizeSnapshot(rows=[],players=[],{seasons=DEFAULT_SEASONS,snapshotDate=new Date().toISOString(),fallbackResolver=null,sourceDataset=SOURCE_DATASET}={}){
  ...
  rows.forEach(row=>{
    ...
    try{records.push(Contract.usageRecord({canonicalPlayerId,providerPlayerId:sourcePlayerId,position:player.pos??player.position,season,week,source:SOURCE,sourceDataset,snapshotDate,aggregationLevel:week===null?'SEASON':'WEEK',stats:translated.stats,fieldMetadata:translated.fieldMetadata,sample:{games:row.games,weeksObserved:week===null?row.weeks_observed:1,minimumSampleStatus:'UNASSESSED',confidence:'UNASSESSED'},matchStatus:'MATCHED',matchConfidence:confidence,identityMethod:method}))}catch(error){quarantined.push({sourcePlayerId,canonicalPlayerId,season,week,reason:'MALFORMED_RECORD',reasonCategory:errorReason(error),field:errorField(error),detail:error.message})}
  });
  const quarantineReasonCounts=countReasons(quarantined,quarantineReason),quarantineFieldReasonCounts=countFieldReasons(quarantined),qualityReasonCounts={...quarantineReasonCounts};
  if(unmatched.length||ambiguous.length)qualityReasonCounts[QUALITY_REASONS.IDENTITY_FAILURE]=unmatched.length+ambiguous.length;
  return Object.freeze({schemaVersion:1,provider:SOURCE,sourceDataset,seasons:Object.freeze([...allowed].sort()),snapshotDate:new Date(snapshotDate).toISOString(),records:Object.freeze(records),matchedRecords:records.length,matchedPlayers:new Set(records.map(record=>record.canonicalPlayerId)).size,unmatched:Object.freeze(unmatched),ambiguous:Object.freeze(ambiguous),quarantined:Object.freeze(quarantined),unmatchedCount:unmatched.length,ambiguousCount:ambiguous.length,quarantinedCount:quarantined.length,quarantineReasonCounts,quarantineFieldReasonCounts,qualityReasonCounts:Object.freeze(qualityReasonCounts)});
}
module.exports=Object.freeze({DEFAULT_SEASONS,SOURCE,SOURCE_DATASET,QUALITY_REASONS,buildIdentityIndex,buildVerifiedFallback,freshness,translateStats,errorReason,errorField,quarantineReason,countReasons,countFieldReasons,normalizeSnapshot});
