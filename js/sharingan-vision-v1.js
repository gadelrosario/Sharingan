...
const SharinganVisionV1 = (() => {
  ...
  function assessTeamNeeds({position, teamsBeforeNext = []}) {
    ...
    const starterTarget = {QB:1,RB:2,WR:3,TE:1}[position];
    const depthTarget = {QB:1,RB:4,WR:5,TE:1}[position];
    ...
    return {teams:teamsBeforeNext.length,starterNeeds,depthNeeds,reason:`${starterNeeds} of ${teamsBeforeNext.length} teams before your next pick still need a starting ${position}; ${depthNeeds} more have room for depth.`};
  }
  ...
  function availabilityForecast({player, availablePlayers = [], picksUntil = 0, tierCliff, run, teamNeeds}) {
    ...
    if (run?.active) urgency += 2; else if (run?.count >= 2) urgency += 1;
    ...
    const reason = `${tierCliff?.remainingInTier ?? 'Unknown'} same-tier alternatives, ${run?.count ?? 0} recent ${position} picks, ${teamNeeds?.starterNeeds ?? 0} upcoming starter needs, and ${turns} picks until your turn.`;
    return {label,urgency,nearbyDepth,reason};
  }

  function opportunityWindow({forecast, tierCliff, run, teamNeeds}) {
    let label;
    if (forecast.label === 'Unlikely Available' || (tierCliff?.nearCliff && (run?.active || teamNeeds?.starterNeeds >= 3))) label = 'Draft Now';
    ...
    const reason = `${forecast.reason} ${tierCliff?.nearCliff ? 'A tier cliff increases urgency.' : 'No immediate tier cliff is detected.'}`;
    return {label,reason};
  }

  function whyNow({player, breakdown = {}, tierCliff, run, teamNeeds}) {
    ...
    return [
      ...
      {label:'Roster needs',text:teamNeeds?.reason || `Upcoming ${position} roster needs are unavailable.`}
    ];
  }
  ...
  function forecast(input) {
    ...
    return {
      player,tierCliff,run,teamNeeds,availability,
      opportunity:opportunityWindow({forecast:availability,tierCliff,run,teamNeeds}),
      whyNow:whyNow({player,breakdown:input.breakdown,tierCliff,run,teamNeeds}),
      whyNot:whyNot({recommended:player,candidates:input.candidates})
    };
  }

  return {forecast,detectTierCliff,detectPositionalRun,assessTeamNeeds,availabilityForecast,opportunityWindow,whyNow,whyNot};
})();
