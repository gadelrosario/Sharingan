(function(root,factory){
  ...
  function pricingFactors(context){
    ...
    const shortTerm=upper(injury.injuryOpportunity)==='SHORT_TERM',restOfSeason=upper(injury.injuryOpportunity)==='REST_OF_SEASON',dropCost=finite(decision.drop?.dropCost),dropState=upper(decision.drop?.state),sharingan=Boolean(context.signals?.sharinganPick),chidori=Boolean(context.signals?.chidori);
    ...
    return frozen({score,quality,need,fit,signal,persistence,scarcity,replacement,competitionPressure,shortTerm,restOfSeason,dropCost,dropState,phase,strategy,sharingan,chidori});
  }
  ...
  function evaluate(context={}){
    ...
    const mainReason=authorityState.authority===AUTHORITY.BLOCKED?authorityState.reason:factors.restOfSeason?'A durable role change and roster fit support paying above an ordinary claim.':factors.need>=3?'The validated move addresses a major roster need.':factors.signal>=3?'Current opportunity evidence supports a stronger claim.':'The opportunity is useful, but future budget flexibility still matters.';
